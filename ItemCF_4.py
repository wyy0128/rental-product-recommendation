...
if "traffic_source" in visits_train.columns and "device_category" in visits_train.columns:
    vmeta = visits_train[
        ["project_id", "visit_id", "traffic_source", "device_category"]
    ].copy()
    ...
    pv_meta = pv.merge(vmeta, on=["project_id", "visit_id"], how="left")
    pv_meta["traffic_source"] = pv_meta["traffic_source"].fillna("NA")
    pv_meta["device_category"] = pv_meta["device_category"].fillna("NA")

    # (project, traffic_source, device)
    popular_by_proj_src_dev = (
        pv_meta.groupby(
            ["project_id", "traffic_source", "device_category"]
        )["product_id"]
        .value_counts()
        .groupby(level=[0, 1, 2])
    ...
    # (project, traffic_source)
    popular_by_proj_src = (
        pv_meta.groupby(
            ["project_id", "traffic_source"]
        )["product_id"]
        .value_counts()
        .groupby(level=[0, 1])
...
# =================== 5) 推荐 + 去重补齐 ===================
def popular_fallback(proj, seen_set, k=6, traffic_source="NA", device_category="NA"):
    recs = []

    # 1) (project, traffic_source, device)
...
def recommend_itemcf(proj: str, vid: str, k=6):
    ...
    for i in seen:
        ...
        for j, s in item_sim.get(i, []):
            if j in seen_set:
                continue
            pop_penalty = 1.0 / np.sqrt(item_cnt.get(j, 1))
            scores[j] += s * w * pop_penalty
...
for proj, vid, src, dev in zip(
    ...
    # 强制兜底补齐
    seen = set(test_session_items.get((proj, vid), []))
    if len(recs) < K_RECS:
        recs = popular_fallback(
            proj, seen, k=K_RECS,
            traffic_source=src,
            device_category=dev
        )
...
def offline_eval_leave_one_out(train_session_items, k=6, sample_sessions=50000, seed=42):
    ...
    for (proj, vid) in keys:
        items = train_session_items[(proj, vid)]
        if len(items) < 2:
            continue
        ...
        # 用“seen”来打分（复用你的 item_sim_by_proj / item_cnt_by_proj / popular_by_proj）
        scores = Counter()
        item_sim = item_sim_by_proj.get(proj, {})
        item_cnt = item_cnt_by_proj.get(proj, Counter())
        seen_set = set(seen)
        ...
        if scores:
            ...
            # 兜底：站点热门
            recs = popular_by_proj.get(proj, [])[:k]
    ...
    print(f"[Offline] Recall@{k}: {recall_at_k:.4f}  (eval sessions={total}, skipped_empty={empty_seen})")
    return recall_at_k

offline_eval_leave_one_out(train_session_items, k=6, sample_sessions=50000)
